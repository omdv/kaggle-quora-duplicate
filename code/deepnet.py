...
x2 = tk.texts_to_sequences(train.question2.values.astype(str))
x2 = sequence.pad_sequences(x2, maxlen=max_len)

# test set
x1p = tk.texts_to_sequences(test.question1.values.astype(str))
x1p = sequence.pad_sequences(x1p, maxlen=max_len)

x2p = tk.texts_to_sequences(test.question2.values.astype(str))
x2p = sequence.pad_sequences(x2p, maxlen=max_len)
...
for line in tqdm(f):
    values = line.decode('utf-8').replace(u'\xa0', u'').split()
    word = values[0]
...
merged_model.add(Merge(
    [model1, model2, model3, model4, model5, model6], mode='concat'))
